...
`kaos-graph` requires Python 3.13 or newer. It publishes the `kaos_graph`
import package. This package publishes CLI entry point(s): `kaos-graph`, `kaos-graph-serve`.
Public extras currently declared: `rdf`.
...
include the `kaos-graph` version, Python version, operating system,
