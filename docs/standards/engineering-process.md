...
This document defines how `kaos-graph` handles features, bugs,
...
- What `kaos-graph` version, platform, Python version, and installation
  method are involved?
- Is there a minimal reproducer?
- Which public API, CLI, integration, packaging, or docs behavior is
  affected?
...
Do not use feature flags to hide unfinished public APIs.
...
- Does it affect performance, memory, errors, output ordering,
  serialization, CLI output, schema output, environment variables, or
  package metadata?
...
- User-visible impact.
- Public API, CLI, schema, environment-variable, or package metadata
  impact.
- Release impact.
...
- Do not discuss suspected vulnerabilities in public issues until they
  are triaged.
- Use the repository's vulnerability reporting process in `SECURITY.md`.
