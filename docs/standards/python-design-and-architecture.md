...
These standards apply to Python code in `kaos-graph`.

`kaos-graph` publishes the `kaos_graph` import package. This package publishes CLI entry point(s): `kaos-graph`, `kaos-graph-serve`.

## Package Shape

- Keep the import package name aligned with the distribution name:
  `kaos-graph` publishes import package `kaos_graph`.
- Declare the public API in `kaos_graph.__all__` when the package has
  a top-level public surface.
- Keep `py.typed` in the wheel for typed packages.
...
- Use Pydantic for external boundaries: configuration, JSON-like inputs,
  API payloads, MCP schemas, and serialized results.
